...
const getAllVideos = asyncHandler(async (req, res) => {
  ...
  let pipeline = [
    ...
    // 3. lookup the Owner field of video and get the user details
    {
      // from user it match the _id of user with Owner field of video and saves as Owner
      $lookup: {
        from: "users",
        localField: "Owner",
        foreignField: "_id",
    ...
    {
      // 4. addFields just add the Owner field to the video document
      //   The $addFields stage with $first ensures that the Owner field becomes a single object rather than an array containing one object,
      //   making it easier to access and use in subsequent steps or when sending the response back to the client.
      $addFields: {
        Owner: {
          $first: "$Owner", // $first: is used to extract the first element of Owner array and convert Owner into a simple object instead of a array
        },
      },
    },
    {
...
const publishAVideo = asyncHandler(async (req, res) => {
  try {
    ...
    // Purpose: some() tests whether at least one element in the array passes the test implemented by the provided function.
    // Returns: A boolean (true or false).
    // Use Case: Use some() when you want to check if any element in the array meets a certain condition.
...
const deleteVideo = asyncHandler(async (req, res) => {
  ...
  // 4. Delete the video document from the database
  await video.remove(); // .remove dont workd with findOne it only works with findById

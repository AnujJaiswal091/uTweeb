...
// 1. get channelName(username) from req.body or req.params
// 2. find the channel(user) using the username and get the channelId
...
const getChannelStats = asyncHandler(async (req, res) => {
  ...
  let { channelName } = req.body || req.params;
  ...
  const totalViewsAndTotalVideos = await Video.aggregate([
    ...
    {
      $group: {
        // group videos by owner and count views and total videos
        _id: "$owner",
...
const getChannelVideos = asyncHandler(async (req, res) => {
  ...
  let pipeline = [
    ...
    {
      // 2.2 unwind the ownerDetails array (Unwind the array to get a single document for each video)
      //   Breaks down an array field into individual documents, where each document contains a single element from the array.
